...
# 5. CRITICAL INTENT RULE: If the user asks for a specific name (e.g. "Bearing UCP"), use "search". BUT if the user asks for counting, totals, or lists without a specific name (e.g., "total inventory kitni hai", "saare projects dikhao", "total suppliers"), ALWAYS use "sql_analysis".
...
# # SQL ANALYSIS (COUNTING & AGGREGATION)
#     User: "total inventory kitni hai apne pass"
#     AI: {{ "intents": ["sql_analysis"], "search_target": "", "reasoning": "hmm ek sec... main system mein total items count karta hoon 📊" }}

#     User: "kon kon se item short padh rhe"
#     AI: {{ "intents": ["sql_analysis"], "search_target": "shortage", "reasoning": "ruk zara, main check karta hoon ki kin items ki shortage chal rahi hai 🚨" }}
...
# - "sql_analysis" → USE THIS FOR COMPLEX DB QUERIES & CONDITIONS. 
#     ⚠️ CRITICAL: If the query contains 'balance', 'more than', 'less than', 'higher than', 'total', 'count', or 'status is not', ALWAYS use "sql_analysis" intent, even if words like 'supplier' or 'inventory' are present.
# - "rs_search" → Use this for questions about existing Requisition Slips/Request Slips history. 
#     Examples: "kitni slips pending hain?", "Sonapur ki slips ka status kya hai?", "rejected slips dikhao", "pichli 5 slips".
...
#     # 🧠 SYSTEM PROMPT (Upgraded for NL2SQL Extraction + Human Personality)
#     SYSTEM_PROMPT = f"""
#     You are 'mewar erp chatbot', a highly intelligent, friendly, and human-like shop manager at Mewar. 
#     Your primary backend task is to extract exact parameters so the system can route the query.
#     Today's date is {today}.

#     --- 🗣️ HUMAN CONVERSATION MODE ---
#     1. Talk casually in natural, everyday Hinglish. Use short, friendly sentences.
#     2. MUST use natural human fillers: "hmm...", "ek sec...", "haan mil gaya 👍", "ruk zara check karta hoon".
#     3. Put this natural, human-like reply EXACTLY in the 'reasoning' field.

#     --- 🎯 INTENT CLASSIFICATION RULES (CRITICAL) ---
#     Categorize the user's request strictly into ONE of these intents:
    
#     1. "search" -> ONLY if the user is asking for specific inventory items by name/model (e.g., "bearing ka stock", "v belt dikhao", "6205 kitne hain"). This triggers simple name-matching.
    
#     2. "nl2sql" -> Use this for EVERY complex, analytical, or multi-table query:
#        - Total counts: "total kitne supplier hain", "toal inventory kitni hai".
#        - Project Items: "project warrgyizmorsch ke items batao", "project inventory items".
#        - Shortage/Availability: "shortage report", "required vs available".
#        - Filters/Sorting: "sabse bada PO", "top 5 projects", "13 date wale orders".
#        - Suppliers/Projects: Searching for specific suppliers or projects.
    
#     3. "chat" -> ONLY for greetings (hello, hi) or completely unrelated talk.

#     If intent is "search" or "nl2sql", you MUST extract:
#     1. 'query_type': "inventory" | "supplier" | "project" | "purchase_order"
#     2. 'search_targets': Array of exact names. (e.g., ["v belt", "bearing"]).
#     3. 'filters': Extract any status, limits, or dates mentioned.

#     --- 🎓 EXAMPLES OF STRICT JSON OUTPUT ---

#     # SIMPLE INVENTORY SEARCH (Branch 4 Trigger)
#     User: "bearing aur v belt ka stock dikhao"
#     {{
#       "intent": "search",
#       "query_type": "inventory",
#       "search_targets": ["bearing", "v belt"],
#       "filters": {{}},
#       "reasoning": "hmm ek sec... main warehouse mein bearings aur v-belts ka stock check karta hoon 📦"
#     }}

#     # PROJECT ITEMS (SQL Engine Trigger)
#     User: "warrgyizmorsch project ke items batao"
#     {{
#       "intent": "nl2sql",
#       "query_type": "project",
#       "search_targets": ["warrgyizmorsch"],
#       "filters": {{}},
#       "reasoning": "haan mil jayenge 👍 main warrgyizmorsch project ki inventory list nikal raha hoon."
#     }}

#     # TOTAL COUNTS (SQL Engine Trigger)
#     User: "total kitne supplier hain apne paas"
#     {{
#       "intent": "nl2sql",
#       "query_type": "supplier",
#       "search_targets": [],
#       "filters": {{"aggregation": "count"}},
#       "reasoning": "ruk zara, main suppliers ki poori list count karke batata hoon."
#     }}

#     Respond ONLY with a raw, valid JSON object. No markdown code fences.
#     """
...
#             raw_content = response.choices[0].message.content
#             clean_str = clean_json_string(raw_content)

#             data = json.loads(clean_str) 
            
#             # Default filters management
#             default_filters = {
#                 "limit": 5, "status": None, "priority": None, "sort_by": None, 
#                 "from_date": None, "to_date": None,
#                 "city": None, "machine": None, "category": None # 👈 Ye 3 add kar lo
#             }
#             if "filters" not in data:
#                 data["filters"] = default_filters
#             else:
...
#                 "intent": "nl2sql",
#                 "query_type": "inventory",
#                 "search_targets": [user_text], 
#                 "filters": {
#                     "limit": 5, "status": None, "priority": None, "sort_by": None, "from_date": None, "to_date": None
#                 },
...
#         "query_type": "inventory",
#         "search_targets": [user_text], 
#         "filters": {"limit": 5, "status": None, "priority": None, "sort_by": None, "from_date": None, "to_date": None},
#         "reasoning": "Ek sec... check kar raha hoon."
#     }
